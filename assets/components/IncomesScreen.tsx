...
const IncomesScreen = () => {
  ...
  const fetchIncomesData = async () => {
    ...
    try {
      ...
      const [totalResponse, chartResponse] = await Promise.all([
        axios.get(`https://backend.money-app-api.com/api/v1/incomes/total?${params}`, { headers: { "x-access-token": token } }),
        axios.get(`https://backend.money-app-api.com/api/v1/incomes/total_by_category?${params}`, { headers: { "x-access-token": token } })
      ]);
  ...
  const fetchIncomesList = async () => {
    ...
    try {
      ...
      const params = buildQueryParams();
      const response = await axios.get(`https://backend.money-app-api.com/api/v1/incomes/list?${params}`, {
  ...
  const deleteIncome = async (incomeId) => {
    try {
      ...
      await axios.delete(`https://backend.money-app-api.com/api/v1/incomes/${incomeId}`, {
        headers: { "x-access-token": token },
      });
